...
function UpdateRegionSettings($GeoID) 
{
  try {
    try {
      ...
      if ((Get-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Control Panel\DeviceRegion" -ErrorAction SilentlyContinue).PSObject.Properties.Name -contains "DeviceRegion") {
        ...
        Write-Host "***Starting AVD AIB CUSTOMIZER PHASE: Set default Language - DeviceRegion registry key doesn't exist."
      }
    }
    catch 
    {
